    ...
    // v14 Intents 사용 - *** 최소한의 Intent로 테스트 ***
    const client = new Client({
        intents: [
            GatewayIntentBits.Guilds,          // 서버 관련 기본 이벤트
            GatewayIntentBits.GuildInteractions // 슬래시 명령어, 버튼 등 상호작용 이벤트
        ]
    });
    ...
    */
    ...
    // 이 코드를 실행했을 때, `BitFieldInvalid` 오류 없이 봇이 정상적으로 시작되는지 확인해보세요. 만약 이 최소한의 Intent 설정으로도 오류가 발생한다면, `discord.js` 라이브러리 설치 자체에 문제가 있거나 다른 환경적인 요인이 있을 수 있습니다.

    // 만약 이 코드로 봇이 **정상적으로 시작된다면**, 문제가 되는 것은 `GuildMessages`, `MessageContent`, `GuildScheduledEvents` 중 하나와 `GuildInteractions` 의 **조합**일 수 있습니다. 이 경우, 다시 원래 코드로 돌아가서 저 3개의 Intent를 하나씩 제거해보면서 어떤 것을 제거했을 때 오류가 사라지는지 확인하는 방식으로 문제를 좁혀나갈 수 있습
